...
const PriceUpload = () => {
  ...
  const deleteFile = async (fileId) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_HOST}/api/files/delete/${fileId}`);
          ...
          <List>
            {files.map((file) => (
              <ListItem key={file._id}>
                ...
                <Button variant="contained" color="error" onClick={() => deleteFile(file._id) } sx={{ marginLeft: 1 }}>
                  Delete
                </Button>
              </ListItem>
            ))}
