...
const SleighSearch = ({ selectedOrders, thisMonthSubmissions, onCheckboxChange, checkboxState, searchQuery }) => {
  ...
  useEffect(() => {
    const fetchThisMonthSubmissions = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_HOST}/api/submissions/search-by-postal-code-Sleigh?query=${searchQuery}`);
